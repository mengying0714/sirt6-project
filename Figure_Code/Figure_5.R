...
df1 <- data.frame(
  group = c(rep("Young", 8), rep("Old control", 12)),
  value = c(9.64, 9.84, 9.79, 10.54, 12.47, 12.19, 11.06, 11.06,
            15.49, 14.60, 19.06, 19.79, 22.96, 23.11, 21.08, 20.89, 21.66, 20.89, 24.62, 22.89)
)

df1$group <- factor(df1$group, levels = c("Young", "Old control"))
...
p <- ggplot(df1, aes(x = group, y = value, color = group)) +
  geom_boxplot(outlier.shape = NA, width = 0.5) +
  geom_jitter(width = 0.1, size = 2) +
  scale_color_manual(values = c("Young" = "#3DBBD4", "Old control" = "#E74C3C")) +
  stat_compare_means(comparisons = list(c("Young", "Old control")),
                     method = "wilcox.test",
                     label = "p.format", size = 7) +
  theme_classic() +
  ...
  labs(x = NULL, y = "Inter-Chrom (%)") +
  ylim(8, 26)
ggsave("boxplot_Young_vs_Old.inter.percentage.pdf",plot = p, width = 4, height = 4.5)
...
df2 <- data.frame(sample = rep(1:6, 2),
                  ...
                  value = c(15.05,19.43,23.04,20.99,21.28,23.76,14.16,19.03,20.83,19.51,19.20,22.42))

df2$group <- factor(df2$group, levels = c("Old control", "Old SIRT6"))

ggplot(df2, aes(x = group, y = value, group = sample)) +
  geom_line(color = "gray75", linewidth = 0.7) +
  geom_point(aes(color = group), size = 6) +
  scale_color_manual(values = c("Old control" = "#EF4B3E", "Old SIRT6" = "#00A88F")) +
  stat_compare_means(aes(group = group), paired = TRUE, method = "wilcox.test",
